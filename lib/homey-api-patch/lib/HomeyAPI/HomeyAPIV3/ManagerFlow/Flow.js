...
class Flow extends Item {
  ...
  async isBroken() {
    ...
    const checkTokens = async card => {
      // Check droptoken
      if (card.droptoken && !tokenIds.includes(card.droptoken)) {
        tokenIds.push(card.droptoken);
      }
    ...
    if (this.trigger) {
      try {
        ...
        if (triggerCard && triggerCard.tokens) {
          if (Array.isArray(triggerCard.tokens)) {
            for (const token of triggerCard.tokens) {
              if (!token) continue;
              const possibleIds = [token.id, token.uri].filter(Boolean);
              for (const possibleId of possibleIds) {
                if (!tokenIds.includes(possibleId)) {
                  tokenIds.push(possibleId);
                }
              }
            }
          } else if (typeof triggerCard.tokens === 'object') {
            for (const [tokenId, token] of Object.entries(triggerCard.tokens)) {
              if (!tokenIds.includes(tokenId)) {
                tokenIds.push(tokenId);
              }
              if (token && token.id && !tokenIds.includes(token.id)) {
